...
const HeroSlider: React.FC = () => {
  ...
  // Cargar imágenes dinámicamente desde la API
  useEffect(() => {
    const loadImages = async () => {
      try {
        const response = await fetch('/api/pc-images');
        const data = await response.json();
        
        if (data.images && data.images.length > 0) {
          setSlides(data.images);
        } else {
          // Fallback a imágenes estáticas si la API falla
          console.warn('No se pudieron cargar las imágenes dinámicamente, usando fallback');
          setSlides([
            { id: 1, src: '/img/pc-hogar-oficina/1.jpg', alt: 'Reparación de computadores en hogar' },
            { id: 2, src: '/img/pc-hogar-oficina/2.jpg', alt: 'Mantenimiento de PC en oficina' },
            { id: 3, src: '/img/pc-hogar-oficina/3.JPG', alt: 'Servicio técnico especializado' },
          ]);
        }
      } catch (error) {
